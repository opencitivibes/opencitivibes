...
| `TOTP_ENCRYPTION_KEY` | 2FA secret encryption key (optional) | See [Two-Factor Authentication](#two-factor-authentication-2fa) |
...
OpenCitiVibes supports optional two-factor authentication (TOTP) for user accounts. Users can enable 2FA in their account settings using any authenticator app (Google Authenticator, Authy, etc.).

#### Configuration

2FA requires a `TOTP_ENCRYPTION_KEY` to encrypt user TOTP secrets at rest. This key is automatically generated by the setup scripts (`setup-staging-vps.sh`, `setup-production-vps.sh`).

**For new deployments:** The setup scripts generate this key automatically.

**For existing deployments:** If you deployed before 2FA was added, generate and add the key manually:
...
docker compose --profile staging restart backend  # or --profile prod
```
...
|-----------|-------------|
| Encryption | TOTP secrets encrypted with Fernet (AES-128-CBC) |
| Storage | Encrypted secrets stored in `User.totp_secret` column |
| Verification | Standard TOTP algorithm (RFC 6238), 30-second window |
| Recovery | Users can disable 2FA if they have access to their email |
...
5. On future logins, prompted for TOTP code after password

#### Security Notes

- **Key rotation:** Changing `TOTP_ENCRYPTION_KEY` invalidates all existing 2FA setups
- **Backup:** Include `TOTP_ENCRYPTION_KEY` in your secrets backup
- **Optional:** 2FA is user-opt-in; the platform works without it if the key is not configured
